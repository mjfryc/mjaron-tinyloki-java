...
public class TinyLokiTest {
    ...
    void dummySendTest() throws InterruptedException {
        TinyLoki tinyLoki = TinyLoki.withUrl("http://localhost/loki/api/v1/push").withLogSender(new DummyLogSender(TinyLoki.DEFAULT_SYNC_TIMEOUT - 100)).withLogMonitor(new VerboseLogMonitor()).withLabelLength(1, 1).withExecutor(new ThreadExecutor(10 * 1000)).start();
        ILogStream abcStream = tinyLoki.stream().info().l("abc", "bcd").build();
        abcStream.log(1, "Hello world.");
